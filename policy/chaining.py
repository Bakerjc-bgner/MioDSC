...
from network.base_net import Classifier,OptionPolicyNetwork
...
class chaining:
    def __init__(self, args):
        ...
        self.global_option = agent.option(self.args)
        ...
        if self.args.alg == 'chaining':
            print('Init alg chaining')
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/rnn_params.pkl'):
                ...
                path_chaining = self.model_dir + '/critic_params.pkl'
                map_location = 'cuda:0' if self.args.cuda else 'cpu'
                self.eval_rnn.load_state_dict(torch.load(path_rnn, map_location=map_location))
                self.eval_critic.load_state_dict(torch.load(path_chaining, map_location=map_location))
                print('Successfully load the model: {} and {}'.format(path_rnn, path_chaining))
            else:
    ...
    def _get_critic_inputs(self, batch, transition_idx, max_episode_len):
        ...
        option_identity = Classifier(self.state)
    ...
    # 获取Option的价值函数输出
    def _get_option_values(self, batch, max_episode_len):
        episode_num = batch['o'].shape[0]
        option_evals, option_targets = [], []
        for transition_idx in range(max_episode_len):
            inputs, inputs_next = self._get_critic_inputs(batch, transition_idx, max_episode_len)
            if self.args.cuda:
                inputs = inputs.cuda()
                inputs_next = inputs_next.cuda()
            # 神经网络输入的是(episode_num * n_agents, inputs)二维数据，得到的是(episode_num * n_agents， n_actions)二维数据
            option_eval = self.eval_critic(inputs)
            option_target = self.target_critic(inputs_next)

            # 把q值的维度重新变回(episode_num, n_agents, n_actions)
            option_eval = option_eval.view(episode_num, self.n_agents, -1)
            option_target = option_target.view(episode_num, self.n_agents, -1)
            option_evals.append(option_eval)
            option_targets.append(option_target)
        # 得的q_evals和q_targets是一个列表，列表里装着max_episode_len个数组，数组的的维度是(episode个数, n_agents，n_actions)
        # 把该列表转化成(episode个数, max_episode_len， n_agents，n_actions)的数组
        option_evals = torch.stack(option_evals, dim=1)
        q_targets = torch.stack(q_targets, dim=1)
        option = OptionPolicyNetwork(self._get_action_prob)
        option.cuda()
        return option_evals, option_targets
